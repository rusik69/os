...
/* ── Shared new-syscall table (file-scope, shared by all three functions) ─ */
#define NEW_SYSCALL_TABLE_SIZE 256
static void *new_syscall_table[NEW_SYSCALL_TABLE_SIZE];
static int new_syscall_table_initialized = 0;

/* ── Helper: check if a syscall nr is handled by the built-in dispatch ─ */
static int syscall_new_is_builtin(int nr)
{
    /* Exhaustive list of SYS_* numbers from syscall.h that the main
     * dispatch in syscall.c handles via its switch statement.
     * Return 1 if nr is a built-in, so we refuse to shadow it. */
    switch (nr) {
    ...
    case 190: case 191: case 192: case 193: case 194: case 195:
    ...
    case 256: case 257: case 258: case 259: case 260: case 261:
    case 262: case 263: case 264: case 265: case 266: case 267:
    case 268: case 269: case 270: case 271: case 272: case 273:
    case 274: case 275: case 276: case 277: case 278: case 279:
    ...
    case 320: case 321: case 322: case 323: case 324: case 325:
    case 326: case 327: case 328: case 329: case 330: case 331:
    case 332: case 333: case 334: case 335: case 336: case 337:
    case 338: case 339: case 340: case 341: case 342: case 343:
    case 344: case 345: case 346: case 347: case 348: case 349:
    case 350: case 351: case 352: case 353: case 354: case 355:
    case 356: case 357: case 358: case 359: case 360: case 361:
    case 362: case 363: case 364: case 365: case 366: case 367:
    case 368: case 369: case 370: case 371: case 372: case 373:
    case 374: case 377: case 378: case 379: case 380: case 381:
    ...
    case 406: case 407: case 428:
    case 504: case 505: case 506: case 507: case 508: case 509:
    case 510: case 511: case 512: case 513: case 555: case 556:
...
{
    if (nr < 0 || nr >= NEW_SYSCALL_TABLE_SIZE || !handler)
        return -EINVAL;

    /* Prevent shadowing a built-in syscall that the main dispatch handles */
    if (syscall_new_is_builtin(nr)) {
        kprintf("[syscall] syscall_new_register: nr=%d would shadow built-in syscall - rejected\n", nr);
        return -EBUSY;
    }

    if (!new_syscall_table_initialized) {
        memset(new_syscall_table, 0, sizeof(new_syscall_table));
        new_syscall_table_initialized = 1;
    }
...
{
    if (nr < 0 || nr >= NEW_SYSCALL_TABLE_SIZE)
        return -EINVAL;

    if (!new_syscall_table_initialized) return -EINVAL;
...
static int syscall_new_invoke(int nr, void *args)
{
    ...
    if (!new_syscall_table_initialized) return 0;
