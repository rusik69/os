 ...
 * at configurable intervals, allowing developers to verify that all
 * allocation failure paths are handled correctly.
 *
 ...
 *   - Runtime control via /sys/kernel/debug/fault_inject/
 *
 * Usage:
 *   # Enable fault injection, fail every 5th kmalloc
 ...
 * Needed to determine when to trigger the next failure. */
