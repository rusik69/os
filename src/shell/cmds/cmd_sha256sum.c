/* cmd_sha256sum.c — SHA-256 hash of a file */
...
static void sha256(const uint8_t *data, uint32_t len, uint8_t out[32]) {
    ...
    buf[rem] = 0x80;
    if (rem < 56) {
        for (uint32_t j = rem+1; j < 56; j++) buf[j] = 0;
    } else {
        for (uint32_t j = rem+1; j < 64; j++) buf[j] = 0;
        sha256_block(h, buf);
        for (uint32_t j = 0; j < 56; j++) buf[j] = 0;
    }
...
void cmd_sha256sum(const char *args) {
    ...
    for (int i = 0; i < 32; i++) kprintf("%x%x", (uint64_t)(digest[i]>>4), (uint64_t)(digest[i]&0xF));
    kprintf("  %s\n", path);
}
