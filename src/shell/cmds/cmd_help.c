...
void cmd_help(void) {
    ...
    kprintf("  ccbuilder- Run build manifest (ccbuilder [-k] <manifest>)\n");
    ...
    kprintf("  fold     - Wrap lines to width (fold [-w N] <file>)\n");
    kprintf("  expand   - Convert tabs to spaces (expand [-t N] <file>)\n");
    kprintf("  comm     - Compare two sorted files (comm [-123] <f1> <f2>)\n");
    kprintf("  split    - Split file into chunks (split [-l N] <file> [prefix])\n");
    kprintf("  which    - Locate a shell command (which <cmd>)\n");
    kprintf("  ln       - Link (copy) a file (ln <src> <dst>)\n");
    kprintf("  true     - Return success\n");
    kprintf("  false    - Return failure\n");
