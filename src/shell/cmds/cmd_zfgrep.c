/* cmd_zfgrep.c — fixed-string grep in compressed files (like grep -F) */
...
void cmd_zfgrep(const char *args) {
    if (!args || !*args) { kprintf("Usage: zfgrep <pattern> <file.gz> [files...]\n"); return; }

    char argbuf[128];
    strncpy(argbuf, args, 127); argbuf[127] = '\0';

    char *pattern = strtok(argbuf, " ");
    if (!pattern) { kprintf("Usage: zfgrep <pattern> <file.gz>\n"); return; }
