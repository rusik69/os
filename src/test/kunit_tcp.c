...
static void tcp_checksum_transport_test(struct kunit *test)
{
    ...
    uint16_t c1 = net_transport_checksum(0x0A000001, 0x0A000002,
                                         IP_PROTO_TCP, payload, sizeof(payload));
    KUNIT_EXPECT_NE(test, (int64_t)c1, (int64_t)0);

    /* Same inputs → same checksum (determinism). */
    uint16_t c1b = net_transport_checksum(0x0A000001, 0x0A000002,
                                          IP_PROTO_TCP, payload, sizeof(payload));
    KUNIT_EXPECT_EQ(test, (int64_t)c1, (int64_t)c1b);

    /* Different src IP → different checksum. */
    uint16_t c2 = net_transport_checksum(0x0A000003, 0x0A000002,
                                         IP_PROTO_TCP, payload, sizeof(payload));
    KUNIT_EXPECT_NE(test, (int64_t)c1, (int64_t)c2);

    /* Different protocol → different checksum. */
    uint16_t c3 = net_transport_checksum(0x0A000001, 0x0A000002,
                                         IP_PROTO_UDP, payload, sizeof(payload));
...
static void tcp_route_longest_prefix_test(struct kunit *test)
{
    ...
    KUNIT_EXPECT_EQ(test, (int64_t)rt_add(0x0A000000, 0xFF000000, 0x01010101, 1), (int64_t)0); /* /8  */
    KUNIT_EXPECT_EQ(test, (int64_t)rt_add(0x0A000200, 0xFFFFFF00, 0x02020202, 2), (int64_t)0); /* /24 */
    KUNIT_EXPECT_EQ(test, (int64_t)rt_add(0x0A00020A, 0xFFFFFFF0, 0x03030303, 3), (int64_t)0); /* /28 */
...
static void tcp_udp_bind_test(struct kunit *test)
{
    ...
    for (int i = 0; i < net_num_udp_bindings; i++) {
        if (net_udp_bindings[i].port == 42001) { found = 1; break; }
    }
...
static void tcp_igmp_join_leave_test(struct kunit *test)
{
    ...
    for (int i = 0; i < IGMP_MAX_GROUPS; i++) {
        if (g[i].in_use && g[i].multiaddr == mreq.imr_multiaddr) { found_idx = i; break; }
    }
...
static void tcp_netfilter_chain_test(struct kunit *test)
{
    ...
    KUNIT_EXPECT_EQ(test,
                    (int64_t)nf_iterate_hooks(NF_INET_LOCAL_IN, NULL, 0),
                    (int64_t)NF_ACCEPT);

    /* Register an accept hook → still ACCEPT. */
    KUNIT_EXPECT_EQ(test,
                    (int64_t)nf_register_hook(NF_INET_LOCAL_IN, test_nf_accept_fn, 0),
                    (int64_t)0);
    KUNIT_EXPECT_EQ(test,
                    (int64_t)nf_iterate_hooks(NF_INET_LOCAL_IN, NULL, 0),
                    (int64_t)NF_ACCEPT);
    ...
    KUNIT_EXPECT_EQ(test,
                    (int64_t)nf_register_hook(NF_INET_LOCAL_IN, test_nf_drop_fn, 100),
                    (int64_t)0);
    KUNIT_EXPECT_EQ(test,
                    (int64_t)nf_iterate_hooks(NF_INET_LOCAL_IN, NULL, 0),
                    (int64_t)NF_DROP);
    ...
    KUNIT_EXPECT_EQ(test,
                    (int64_t)nf_hook_traverse(NF_INET_LOCAL_IN, NULL, NULL, 0),
                    (int64_t)(-1)); /* -1 == dropped */
...
static void tcp_netfilter_priority_test(struct kunit *test)
{
    ...
    KUNIT_EXPECT_EQ(test,
                    (int64_t)nf_register_hook(NF_INET_FORWARD, test_nf_accept_fn, 10),
                    (int64_t)0);
    KUNIT_EXPECT_EQ(test,
                    (int64_t)nf_register_hook(NF_INET_FORWARD, test_nf_accept_fn, 50),
                    (int64_t)0);
    KUNIT_EXPECT_EQ(test,
                    (int64_t)nf_iterate_hooks(NF_INET_FORWARD, NULL, 0),
                    (int64_t)NF_ACCEPT);
...
static const struct kunit_case tcp_test_cases[] = {
    KUNIT_CASE(tcp_checksum_ip_test),
