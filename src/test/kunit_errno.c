...
static void strerror_streams_test(struct kunit *test) {
    ...
    KUNIT_EXPECT_TRUE(test,
                      errno_str_eq(test, ERESTART, "Interrupted system call should be restarted"));
...
static void strerror_socket_test(struct kunit *test) {
    ...
    KUNIT_EXPECT_TRUE(
        test, errno_str_eq(test, EOPNOTSUPP, "Operation not supported on transport endpoint"));
    KUNIT_EXPECT_TRUE(test, errno_str_eq(test, EPFNOSUPPORT, "Protocol family not supported"));
    KUNIT_EXPECT_TRUE(test,
                      errno_str_eq(test, EAFNOSUPPORT, "Address family not supported by protocol"));
    ...
    KUNIT_EXPECT_TRUE(test,
                      errno_str_eq(test, ENETRESET, "Network dropped connection because of reset"));
    ...
    KUNIT_EXPECT_TRUE(
        test, errno_str_eq(test, ESHUTDOWN, "Cannot send after transport endpoint shutdown"));
...
static struct kunit_case errno_numeric_test_cases[] = {KUNIT_CASE(errno_constants_test), {0}};

static struct kunit_case strerror_core_cases[] = {KUNIT_CASE(strerror_core_test),
                                                  KUNIT_CASE(strerror_streams_test),
                                                  KUNIT_CASE(strerror_socket_test),
                                                  KUNIT_CASE(strerror_late_test),
                                                  {0}};

static struct kunit_case strerror_edge_cases[] = {KUNIT_CASE(strerror_edge_cases_test), {0}};

static struct kunit_case errno_api_cases[] = {KUNIT_CASE(errno_location_api_test), {0}};
...
void kunit_errno_register(void) {
    /* ── Errno numeric constants ── */
    {
        int ci = 0;
        for (int i = 0;
             i < (int)(sizeof(errno_numeric_test_cases) / sizeof(errno_numeric_test_cases[0])) &&
             errno_numeric_test_cases[i].run != NULL;
             i++) {
    ...
    {
        int ci = 0;
        for (int i = 0; i < (int)(sizeof(strerror_core_cases) / sizeof(strerror_core_cases[0])) &&
                        strerror_core_cases[i].run != NULL;
             i++) {
    ...
    {
        int ci = 0;
        for (int i = 0; i < (int)(sizeof(strerror_edge_cases) / sizeof(strerror_edge_cases[0])) &&
                        strerror_edge_cases[i].run != NULL;
             i++) {
    ...
    {
        int ci = 0;
        for (int i = 0; i < (int)(sizeof(errno_api_cases) / sizeof(errno_api_cases[0])) &&
                        errno_api_cases[i].run != NULL;
             i++) {
    ...
    kprintf("[KUnit] Errno test suites registered (errno_numeric, strerror, strerror_edge, "
            "errno_api)\n");
}
