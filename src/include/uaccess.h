...
int __must_check copy_from_user(void *dst, uint64_t __user src_user, size_t n);

/* Copy to user-space from kernel buffer.  Returns 0 on success, -EFAULT on error. */
int __must_check copy_to_user(uint64_t __user dst_user, const void *src, size_t n);
