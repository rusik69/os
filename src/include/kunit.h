...
/* Parameter forward-declaration for fuzz callback typedef */
struct kunit_param;

/* Fuzz parameter generator callback.
 * Called for each fuzz iteration to produce a random parameter value.
 * The callback fills @out_param with a name and value.  The value
 * must remain valid for the duration of the test function call.
 * Test authors may use rng_get_u32(), rng_fill_buf(), or the
 * iteration number for deterministic fuzzing. */
...
/* Enable or disable fuzz mode.
 * When enabled, test cases with a fuzz generator run @fuzz_iters
 * additional iterations with randomly generated parameters. */
void kunit_set_fuzz_enabled(int enabled);

/* Returns non-zero if fuzz mode is enabled. */
int kunit_get_fuzz_enabled(void);

/* Set the default number of fuzz iterations per test case.
 * Individual test cases can override via fuzz_iters field. */
void kunit_set_fuzz_iterations(int iters);

/* Get the current default fuzz iteration count. */
int kunit_get_fuzz_iterations(void);

/* Set the fuzz deterministic seed (for reproducibility).
 * The seed is mixed with the suite/test name and iteration number
 * to produce deterministic parameter sequences. */
void kunit_set_fuzz_seed(uint32_t seed);

/* Get the current fuzz PRNG seed. */
uint32_t kunit_get_fuzz_seed(void);

/* ── Fuzz helper functions (available in fuzz callbacks) ──────────────── */

/* Return a random u32 in [0, max_val) using the kernel RNG.
 * Results differ between runs.  For reproducible fuzzing, generate
 * values deterministically from the iteration number. */
uint32_t kunit_fuzz_rand_u32(uint32_t max_val);

/* Fill a buffer with random bytes using the kernel RNG. */
...
/* Declare a test case with a fuzz parameter generator.
 * When fuzz mode is enabled, the test runs @fuzz_iters additional
 * iterations with parameters generated by @fuzz_fun.
 * Defaults to KUNIT_STRESS_HEAVY because fuzzing is expensive. */
...
/* Like KUNIT_CASE_FUZZ but with explicit stress level. */
