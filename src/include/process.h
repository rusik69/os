...
#define fd_sync_from_shared(pfd) \
    do { \
        if (unlikely((pfd)->shared_offset)) \
            (pfd)->offset = *(pfd)->shared_offset; \
    } while (0)
#define fd_sync_to_shared(pfd) \
    do { \
        if (unlikely((pfd)->shared_offset)) \
            *(pfd)->shared_offset = (pfd)->offset; \
    } while (0)
...
struct process {
    ...
    uint64_t
        sig_sa_mask[PROCESS_SIG_MAX]; /* per-signal sa_mask (signals to block during handler) */
    /* Ring 3 support */
    ...
    uint8_t sched_boost_on_wake; /* 1: vruntime reset below min on wake */
...
#define PROCESS_MAX_HELD_MUTEXES 4
    ...
    uint64_t core_sched_cookie; /* 0 = no restriction; non-zero = must only share core with same
                                   cookie */
...
int __must_check
process_fork(void); /* fork current process, returns child PID, child starts in fork_child_entry */
