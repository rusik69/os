...
/* ── Handshake init sender_index replay tracking ─────────────────── */
/* Track recently-seen sender_index values from handshake initiation
 * messages to detect and reject replays within the timestamp window.
 * Entries expire after WG_SI_EXPIRY_TICKS (~120 seconds at 100 Hz). */
#define WG_SI_TRACKER_SIZE  16
#define WG_SI_EXPIRY_TICKS  12000

struct wg_si_tracker {
    uint32_t sender_index;
    uint64_t seen_time;
    int      active;
};

static struct wg_si_tracker wg_si_track[WG_SI_TRACKER_SIZE];

/* Check whether a sender_index has been seen recently (anti-replay).
 * Returns 0 if the sender_index is new (not seen), or -EBADMSG if
 * it matches a recently-tracked entry (suspected replay). */
static int wg_si_check(uint32_t sender_index)
{
    uint64_t now = timer_get_ticks();

    for (int i = 0; i < WG_SI_TRACKER_SIZE; i++) {
        if (wg_si_track[i].active &&
            wg_si_track[i].sender_index == sender_index) {
            /* Expired entries are considered inactive */
            if (now - wg_si_track[i].seen_time > WG_SI_EXPIRY_TICKS)
                continue;
            return -EBADMSG;
        }
    }
...
/* Record a sender_index so that future messages with the same
 * value are rejected as replays.  Evicts expired or oldest entries. */
static void wg_si_track_add(uint32_t sender_index)
{
    uint64_t now = timer_get_ticks();
    int oldest = 0;
    ...
    for (int i = 0; i < WG_SI_TRACKER_SIZE; i++) {
        if (!wg_si_track[i].active) {
            /* Empty slot — use it */
            wg_si_track[i].sender_index = sender_index;
            wg_si_track[i].seen_time = now;
            wg_si_track[i].active = 1;
            return;
        }
        if (now - wg_si_track[i].seen_time > WG_SI_EXPIRY_TICKS) {
            /* Expired slot — recycle */
            wg_si_track[i].sender_index = sender_index;
            wg_si_track[i].seen_time = now;
            wg_si_track[i].active = 1;
            return;
        }
        if (wg_si_track[i].seen_time < oldest_time) {
            oldest_time = wg_si_track[i].seen_time;
            oldest = i;
        }
    }

    /* All slots full and active — overwrite the oldest entry */
    wg_si_track[oldest].sender_index = sender_index;
    wg_si_track[oldest].seen_time = now;
    wg_si_track[oldest].active = 1;
}
                                   ...
                                   uint32_t src_ip, uint16_t src_port)
{
    ...
    /* Step 6b: Validate sender_index for replay protection.  The sender_index
     * is a random 32-bit value generated by the initiator; if we've seen this
     * same value recently, it is almost certainly a replayed handshake init. */
    if (wg_si_check(sender_index) < 0) {
        kprintf("[WG] Init: sender_index %u replayed, rejecting handshake init\n",
                (unsigned)sender_index);
        return -EBADMSG;
    }
    wg_si_track_add(sender_index);
