...
static int tls_send_new_session_ticket(struct tls_conn *conn, uint8_t *out, int out_cap);

/* Renegotiation helper (defined below, called from tls_handshake_step) */
int tls_handle_renegotiation_client_hello(struct tls_conn *conn, const uint8_t *body, int body_len);
 ...
 */
int tls_hs_build_frame(uint8_t msg_type, const uint8_t *body, int body_len, uint8_t *out,
                       int out_cap) {
 ...
 */
int tls_hs_parse_header(const uint8_t *in, int in_len, uint8_t *msg_type, int *body_len) {
 ...
 */
int tls_build_client_hello(struct tls_conn *conn, const uint16_t *ciphers, int num_ciphers,
                           const uint8_t *session_id, int session_id_len, uint8_t *out,
                           int out_cap) {
    int offset = 0;
    ...
    if (conn->alpn_num_protos > 0) {
        uint8_t ext_buf[2 + 2 + 2 + TLS_ALPN_MAX_PROTOCOLS * (1 + TLS_ALPN_MAX_NAME_LEN)];
        int ext_len;
        int needed;

        ext_len = tls_build_alpn_ext((const uint8_t(*)[TLS_ALPN_MAX_NAME_LEN])conn->alpn_protos,
                                     conn->alpn_proto_lens, conn->alpn_num_protos, ext_buf,
                                     (int)sizeof(ext_buf));
        if (ext_len > 0) {
    ...
    if (conn->sni_hostname_len > 0) {
        ...
        ext_len = tls_build_sni_ext((const char *)conn->sni_hostname, (int)conn->sni_hostname_len,
 ...
 * a negative errno.
 */
int tls_parse_client_hello(struct tls_conn *conn, const uint8_t *body, int body_len,
                           uint16_t *cipher_choices, int *num_choices, int max_choices) {
    int offset = 0;
    uint16_t ver;
    int sid_len;
    ...
    *num_choices = num_ciphers;
    ...
    if (offset < body_len) {
        ...
        if (offset + 2 > body_len)
            return -EINVAL;
        ext_total_len = ((int)body[offset] << 8) | (int)body[offset + 1];
        offset += 2;
        ...
        while (ext_offset + 4 <= ext_total_len) {
            ...
            ext_data_len =
                ((int)body[offset + ext_offset + 2] << 8) | (int)body[offset + ext_offset + 3];

            if (ext_data_len < 0 || ext_offset + 4 + ext_data_len > ext_total_len)
                break;

            /* ── ALPN Extension (TLS_EXT_ALPN = 0x0010) ─── */
            if (ext_type == TLS_EXT_ALPN && ext_data_len > 0) {
                tls_parse_alpn_ext(body + offset + ext_offset + 4, ext_data_len, conn, NULL, NULL,
                                   0);
            }

            /* ── SNI Extension (TLS_EXT_SERVER_NAME = 0x0000) ─── */
            if (ext_type == TLS_EXT_SERVER_NAME && ext_data_len > 0) {
                tls_parse_sni_ext(body + offset + ext_offset + 4, ext_data_len, conn);
            }
 ...
 */
int tls_build_server_hello(struct tls_conn *conn, uint16_t selected_cipher,
                           const uint8_t *session_id, int session_id_len, uint8_t *out,
                           int out_cap) {
    int offset = 0;
    uint16_t version_wire;
    ...
    if (conn->alpn_negotiated && conn->alpn_selected_len > 0) {
        ...
        ext_len =
            tls_build_alpn_ext((const uint8_t(*)[TLS_ALPN_MAX_NAME_LEN]) & conn->alpn_selected,
                               &conn->alpn_selected_len, 1, ext_buf, (int)sizeof(ext_buf));
        if (ext_len > 0) {
 ...
 * a negative errno.
 */
int tls_parse_server_hello(struct tls_conn *conn, const uint8_t *body, int body_len) {
    int offset = 0;
    uint16_t ver;
    int sid_len;
    uint16_t cipher;
 ...
 */
int tls_handshake_step(struct tls_conn *conn, enum tls_hs_state *new_state, const uint8_t *in,
    ...
    if (conn->is_client) {
        ...
        case TLS_HS_IDLE: {
            ...
            uint16_t ciphers[] = {
                ...
                TLS_CHACHA20_POLY1305_SHA256, TLS_ECDHE_RSA_WITH_AES_128_GCM,
                TLS_ECDHE_RSA_WITH_CHACHA20,  TLS_DHE_RSA_WITH_AES_128_GCM,
            };
            ...
            body_len =
                tls_build_client_hello(conn, ciphers, num_ciphers, NULL, 0, body, sizeof(body));
            if (body_len < 0)
                goto error;

            ret = tls_hs_build_frame(TLS_HT_CLIENT_HELLO, body, body_len, out, out_cap);
            if (ret < 0)
                goto error;
        ...
        case TLS_HS_CLIENT_HELLO_SENT: {
            ...
            ret = tls_parse_server_hello(conn, in + TLS_HANDSHAKE_HEADER_LEN, hs_body_len);
            if (ret < 0)
                goto error;
            ...
            if (conn->alpn_negotiated && conn->alpn_selected_len > 0) {
                kprintf("[tls] client: ALPN negotiated "
                        "(%.*s)\n",
                        (int)conn->alpn_selected_len, (const char *)conn->alpn_selected);
            }
        ...
        case TLS_HS_SERVER_PARAMS_SENT: {
            ...
            if (hs_msg_type == TLS_HT_CERTIFICATE) {
                ...
                ret = tls_parse_certificate_msg(in + TLS_HANDSHAKE_HEADER_LEN, hs_body_len, certs,
                                                TLS_MAX_CERT_CHAIN_DEPTH, &num_certs);
                if (ret < 0) {
                ...
                if (num_certs > 0) {
                    ...
                    ret = x509_parse_cert(certs[0].data, certs[0].data_len, &xcert);
                    if (ret == 0)
        ...
        case TLS_HS_TICKET_SENT: {
            ...
            if (in && in_len > 0) {
                ...
                ret = tls_parse_new_session_ticket(in + TLS_HANDSHAKE_HEADER_LEN, hs_body_len,
        ...
        case TLS_HS_CONNECTED: {
            ...
            if (in && in_len > 0 && conn->renego_allowed && conn->version <= TLS_VER_1_2) {
                int hr_ret;
                uint8_t hs_mt;
                int hs_bl;

                hr_ret = tls_hs_parse_header(in, in_len, &hs_mt, &hs_bl);
                if (hr_ret > 0 && hs_mt == TLS_HT_HELLO_REQUEST && hs_bl == 0) {
                    ...
                    ret = tls_conn_request_renegotiate(conn, out, out_cap);
                    if (ret < 0)
                        goto error;
        ...
        case TLS_HS_IDLE: {
            ...
            ret = tls_parse_client_hello(conn, in + TLS_HANDSHAKE_HEADER_LEN, hs_body_len,
                                         cipher_choices, &num_choices, 64);
            if (ret < 0)
                goto error;

            /* Negotiate cipher suite using server preference order */
            ret = tls_negotiate_cipher_suite(cipher_choices, num_choices, NULL, 0);
            if (ret < 0) {
            ...
            /* Build ServerHello */
            srv_body_len = tls_build_server_hello(conn, selected, conn->session_id,
                                                  conn->session_id_len, srv_body, sizeof(srv_body));
            if (srv_body_len < 0)
                goto error;

            ret = tls_hs_build_frame(TLS_HT_SERVER_HELLO, srv_body, srv_body_len, out, out_cap);
            if (ret < 0)
                goto error;
        ...
        case TLS_HS_SERVER_PARAMS_SENT: {
            ...
            ret = tls_hs_build_frame(TLS_HT_CERTIFICATE, body, body_len, out, out_cap);
            if (ret < 0)
                goto error;
        ...
        case TLS_HS_CONNECTED: {
            ...
            if (!conn->renego_allowed || conn->version >= TLS_VER_1_3)
            ...
            /* Validate renegotiation_info extension if present */
            {
                ...
                rnego_ret = tls_handle_renegotiation_client_hello(
                    conn, in + TLS_HANDSHAKE_HEADER_LEN, hs_body_len);
                if (rnego_ret < 0) {
...
void tls_hkdf_extract(const uint8_t *salt, int salt_len, const uint8_t *ikm, int ikm_len,
                      uint8_t *prk) {
    ...
    hmac_sha256(salt, (size_t)salt_len, ikm, (size_t)ikm_len, prk);
}

void tls_hkdf_expand(const uint8_t *prk, int prk_len, const uint8_t *info, int info_len,
    ...
    for (i = 1; generated < out_len; i++) {
        ...
        /* T(i) = HMAC-SHA256(PRK, T(i-1) || info || i) */
        hmac_sha256(prk, (size_t)prk_len, buf, (size_t)(t_len + info_copy_len + 1), t);
        t_len = 32;
...
void tls_hkdf_expand_label(const uint8_t *secret, int secret_len, const char *label,
                           const uint8_t *context, int context_len, uint8_t *out, int out_len) {
    ...
    /* Perform HKDF-Expand */
    tls_hkdf_expand(secret, secret_len, hkdf_label, offset, out, out_len);
}
...
void tls_derive_secret(const uint8_t *secret, const char *label, const uint8_t *transcript_hash,
                       uint8_t *out) {
    if (!secret || !label || !out)
        return;

    tls_hkdf_expand_label(secret, 32, label, transcript_hash, 32, out, 32);
}
...
int tls_build_hello_request(uint8_t *out, int out_cap) {
    ...
    return tls_hs_build_frame(TLS_HT_HELLO_REQUEST, NULL, 0, out, out_cap);
}
 ...
 */
int tls_build_renegotiation_info_ext(const uint8_t *client_verify_data, int client_verify_len,
                                     const uint8_t *server_verify_data, int server_verify_len,
    ...
    if (client_verify_len > 0) {
        memcpy(out + offset, client_verify_data, (size_t)client_verify_len);
        offset += client_verify_len;
    }

    /* Server verify data */
    if (server_verify_len > 0) {
        memcpy(out + offset, server_verify_data, (size_t)server_verify_len);
...
int tls_parse_renegotiation_info_ext(const uint8_t *ext_body, int ext_body_len,
                                     const uint8_t **verify_data, int *verify_data_len) {
    int data_len;
 ...
 */
int tls_conn_request_renegotiate(struct tls_conn *conn, uint8_t *out, int out_cap) {
    int ret;
    ...
    if (conn->is_client) {
        ...
        ch_body_len = tls_build_client_hello(conn, ciphers, num_ciphers, conn->session_id,
                                             conn->session_id_len, ch_body, sizeof(ch_body));
        if (ch_body_len < 0)
            return ch_body_len;

        /* Append renegotiation_info extension */
        ext_len = tls_build_renegotiation_info_ext(
            conn->renego_client_verify_data, TLS_RENEGO_VERIFY_DATA_LEN,
            conn->renego_server_verify_data, TLS_RENEGO_VERIFY_DATA_LEN, ext_buf, sizeof(ext_buf));
        if (ext_len < 0)
            return ext_len;
        ...
        ret = tls_hs_build_frame(TLS_HT_CLIENT_HELLO, ch_body, ch_body_len, out, out_cap);
        if (ret < 0)
            return ret;
 ...
 */
int tls_handle_renegotiation_client_hello(struct tls_conn *conn, const uint8_t *body,
                                          int body_len) {
    (void)conn;
    (void)body;
    (void)body_len;
...
int tls_build_alpn_ext(const uint8_t (*protos)[TLS_ALPN_MAX_NAME_LEN], const uint8_t *proto_lens,
                       int num_protos, uint8_t *out, int out_cap) {
    int offset = 0;
    int body_len;
    int list_len;
    int i;
...
int tls_parse_alpn_ext(const uint8_t *ext_body, int ext_body_len, struct tls_conn *conn,
    ...
    while (offset < 2 + list_len) {
        ...
        if (server_protos == NULL || num_server <= 0) {
            ...
            memcpy(conn->alpn_selected, ext_body + offset, (size_t)cmp_len);
        ...
        for (i = 0; i < num_server; i++) {
            ...
            if (sl == name_len &&
                memcmp(server_protos[i], ext_body + offset, (size_t)name_len) == 0) {
                conn->alpn_selected_len = name_len;
                memcpy(conn->alpn_selected, server_protos[i], (size_t)name_len);
                conn->alpn_negotiated = 1;
                return 0;
            }
        }
...
int tls_alpn_set_protocols(struct tls_conn *conn, const uint8_t (*protos)[TLS_ALPN_MAX_NAME_LEN],
...
const uint8_t *tls_alpn_get_selected(const struct tls_conn *conn, uint8_t *len) {
    if (!conn || !len)
        return NULL;
...
int tls_build_sni_ext(const char *hostname, int hostname_len, uint8_t *out, int out_cap) {
    int offset = 0;
    int body_len;
...
int tls_parse_sni_ext(const uint8_t *ext_body, int ext_body_len, struct tls_conn *conn) {
    int offset = 0;
    int list_len;
    int name_type;
    int name_len;
    ...
    if (name_type == 0) {
        ...
        memcpy(conn->sni_hostname, ext_body + offset, (size_t)name_len);
...
int tls_sni_set_hostname(struct tls_conn *conn, const char *hostname, int len) {
    if (!conn)
        return -EINVAL;
...
int tls_derive_early_traffic_keys(struct tls_conn *conn, const struct tls_psk *psk,
                                  const uint8_t *client_hello, int client_hello_len) {
    ...
    if (psk && psk->valid && psk->identity_len > 0) {
        tls_hkdf_extract(zero_salt, sizeof(zero_salt), psk->secret, sizeof(psk->secret),
                         early_secret);

        memcpy(conn->psk.secret, psk->secret, sizeof(conn->psk.secret));
        conn->psk.identity_len = psk->identity_len;
        if (psk->identity_len > 0)
            memcpy(conn->psk.identity, psk->identity, (size_t)psk->identity_len);
        ...
        tls_hkdf_extract(zero_salt, sizeof(zero_salt), zero_ikm, sizeof(zero_ikm), early_secret);
        conn->has_psk = 0;
    }
     ...
     */
    tls_derive_secret(early_secret, "c e traffic", client_hello_hash, conn->early_wstate.enc_key);

    /* 4. Derive early_exporter_master_secret
     *    (stored in early_rstate as placeholder for server-side use)
     */
    tls_derive_secret(early_secret, "e exp master", client_hello_hash, conn->early_rstate.enc_key);
    ...
    if (conn->has_psk) {
        conn->max_early_data =
            (psk && psk->max_early_data > 0) ? psk->max_early_data : TLS_MAX_EARLY_DATA_SIZE;
    } else {
...
int tls_build_end_of_early_data(uint8_t *out, int out_cap) {
    return tls_hs_build_frame(TLS_HT_END_OF_EARLY_DATA, NULL, 0, out, out_cap);
}
...
int tls_build_early_data_ext(uint32_t max_early_data_size, uint8_t *out, int out_cap) {
    int ext_data_len;
...
int tls_early_data_encrypt(struct tls_conn *conn, const uint8_t *data, int data_len, uint8_t *out,
                           int out_cap) {
    int offset = 0;
    int total_written = 0;
    ...
    if (data_len <= 0)
        return 0;
    if (conn->max_early_data > 0 && (uint32_t)data_len > conn->max_early_data)
        return -EMSGSIZE;
    ...
    while (offset < data_len) {
        ...
        ret =
            tls_record_encrypt(&conn->early_wstate, TLS_CT_APPLICATION_DATA, conn->version,
                               data + offset, chunk, out + total_written, out_cap - total_written);
        if (ret < 0)
            return ret;
...
int tls_early_data_decrypt(struct tls_conn *conn, const uint8_t *in, int in_len, uint8_t *data,
                           int data_cap) {
...

    {
        ...
        ret = tls_record_decrypt(&conn->early_rstate, hdr, cipher_body, cipher_len, data, data_cap);
        if (ret < 0)
            return ret;
    }
...
static int tls_send_new_session_ticket(struct tls_conn *conn, uint8_t *out, int out_cap) {
    ...
    if (conn->session_id_len > 0)
        memcpy(sess_data.session_id, conn->session_id, (size_t)conn->session_id_len);
    /* Use the handshake transcript as PSK secret filler */
    memcpy(sess_data.psk_secret, conn->transcript_hash, sizeof(sess_data.psk_secret));
    ...
    ticket_len = tls_ticket_create(ticket_ctx, &sess_data, ticket_buf, sizeof(ticket_buf),
    ...
    nst_body_len = tls_build_new_session_ticket(ticket_buf, ticket_len, TLS_TICKET_LIFETIME_DEFAULT,
                                                ticket_age_add, nonce_buf, TLS_TICKET_NONCE_LEN,
                                                nst_body, sizeof(nst_body));
    if (nst_body_len < 0)
        return nst_body_len;

    /* Wrap in handshake frame */
    ret = tls_hs_build_frame(TLS_HT_NEW_SESSION_TICKET, nst_body, nst_body_len, out, out_cap);
    return ret;
}
...
#define NUM_CIPHER_SUITES ((int)(sizeof(tls_cipher_suites) / sizeof(tls_cipher_suites[0])))

/* ── Cipher Suite Lookup ──────────────────────────────────────────────── */

const struct tls_cipher_suite_info *tls_cipher_suite_lookup(uint16_t cipher_suite) {
