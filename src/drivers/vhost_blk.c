...
/* Lazily allocate the 32 MB backing store on first request.  The default
 * backing is registered at boot without RAM (no real vhost-blk hardware
 * in most VMs) to avoid wasting 32 MB; the store is created on demand. */
static int vhost_blk_ensure_backing(void) {
    if (vhost_blk_backing.data)
        return 0;

    uint64_t num_sectors = (32ULL * 1024 * 1024) / VHOST_BLK_SECTOR_SIZE;
    uint64_t num_pages = (num_sectors * VHOST_BLK_SECTOR_SIZE + PAGE_SIZE - 1) / PAGE_SIZE;
    uint64_t blk_mem = (uint64_t)pmm_alloc_frames((size_t)num_pages);
    if (!blk_mem) {
        kprintf("[vhost-blk] Failed to allocate backing storage on demand\n");
        return -1;
    }
...
static int vhost_blk_process_request(struct vhost_blk_req_hdr *hdr, uint8_t *data_buf,
                                     uint8_t *status) {
    ...
    /* Ensure backing RAM exists before touching bak->data */
    if (vhost_blk_ensure_backing() < 0) {
        *status = VHOST_BLK_S_IOERR;
        return -1;
    }
    if (!bak->data) {
    ...
    case VHOST_BLK_T_DISCARD: {
        ...
        struct vhost_blk_discard_desc *desc = (struct vhost_blk_discard_desc *)data_buf;
        ...
        if (desc->num_sectors == 0 || desc->sector > UINT64_MAX / VHOST_BLK_SECTOR_SIZE ||
...
int vhost_blk_set_backing(struct vhost_blk_backing *bak) {
    ...
    kprintf("[vhost-blk] backing store set: %llu sectors, readonly=%d, "
            "serial=%s\n",
            vhost_blk_backing.num_sectors, vhost_blk_backing.readonly, vhost_blk_backing.serial);
    return 0;
}
...
int vhost_blk_init(void) {
    /* Backing store: 32 MB by default, allocated lazily on first
     * request (no real vhost-blk hardware in most VMs — pre-allocating
     * 32 MB at boot wasted RAM).  vhost_blk_set_backing() accepts a
     * NULL data pointer and vhost_blk_process_request() allocates the
     * store on demand. */
    uint64_t num_sectors = (32ULL * 1024 * 1024) / VHOST_BLK_SECTOR_SIZE;
    ...
    kprintf("[vhost-blk] vhost block device initialized: "
            ...
            num_sectors, (num_sectors * VHOST_BLK_SECTOR_SIZE) / (1024 * 1024), bak.serial);
    return 0;
}
#include "module.h"
