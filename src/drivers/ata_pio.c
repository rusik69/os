...
int ata_pio_read_sector(int bus, int master, uint32_t lba, uint16_t *buf) {
    ...
    /* ── Debug: trace the module-read region (stall root-cause) ── */
    if (lba >= 8600 && lba < 9000)
        kprintf("[ata] read lba=%u attempt=0\n", (unsigned int)lba);
...
int ata_pio_read_sectors(int bus, int master, uint32_t lba, uint8_t count, uint16_t *buf) {
    uint16_t dh_port = ATA_PIO_DRIVE_HEAD(bus);
    int ret;
    unsigned int s;
    ...
    for (s = 0; s < count; s++) {
        ...
        ret = ata_pio_read_data(bus, buf + s * ATA_SECTOR_WORDS, ATA_SECTOR_WORDS);
        if (ret < 0)
            return ret;
    }
...
int ata_pio_write_sector(int bus, int master, uint32_t lba, const uint16_t *buf) {
    uint16_t dh_port = ATA_PIO_DRIVE_HEAD(bus);
    int ret;
...
int ata_pio_write_sectors(int bus, int master, uint32_t lba, uint8_t count, const uint16_t *buf) {
    uint16_t dh_port = ATA_PIO_DRIVE_HEAD(bus);
    int ret;
    unsigned int s;
    ...
    for (s = 0; s < count; s++) {
        ...
        ret = ata_pio_write_data(bus, buf + s * ATA_SECTOR_WORDS, ATA_SECTOR_WORDS);
        if (ret < 0)
            return ret;
    }
