...
static uint64_t aml_read_le64(const uint8_t *data, uint32_t max_len, uint32_t offset, int bytes);
 ...
 */
static uint32_t aml_decode_pkg_length(const uint8_t *data, uint32_t max_len, uint32_t offset,
                                      uint32_t *bytes_used) {
    uint32_t result = 0;
 ...
 */
static int aml_read_nameseg(const uint8_t *data, uint32_t max_len, uint32_t offset, char name[4]) {
    if (offset + 4 > max_len) {
        name[0] = '?';
        name[1] = '?';
        name[2] = '?';
 ...
 * 0 on error.
 */
static uint32_t aml_skip_name_string(const uint8_t *data, uint32_t max_len, uint32_t offset) {
    uint32_t o = offset;
 ...
 */
static uint32_t aml_read_last_nameseg(const uint8_t *data, uint32_t max_len, uint32_t offset,
                                      char name[4]) {
    uint32_t o = offset;
 ...
 */
static int aml_add_node(const char name[4], uint8_t type, uint16_t parent, const uint8_t *aml_start,
                        uint32_t aml_length, uint8_t ssdt_index) {
    ...
    /* Link as child of parent.  parent must be an EXISTING node, i.e.
     * its index must be strictly less than count BEFORE this node is
     * appended (self-parenting would create a cyclic child chain). */
    if (parent != 0xFFFF && parent < idx) {
...
static int aml_find_child(uint16_t parent, const char name[4]) {
    ...
    /* Bound the sibling walk: a valid namespace has at most g_ns.count
     * nodes, so more hops than that means a corrupt/cyclic chain.  Also
     * reject indices past the physical array (corrupt count). */
    while (child != 0xFFFF && hops++ <= g_ns.count && child >= 0 && child < ACPI_NS_MAX_NODES) {
...
/*
 * Bounded sibling walk: look for a child of `parent` whose NameSeg
 * matches `name`.  Returns the node index or -1.  The hop limit turns
 * a corrupt/cyclic next_sibling chain into a clean miss instead of an
 * infinite loop.
 */
static int aml_find_child_safe(int parent, const char *name) {
    int hops = 0;
    int child;

    if (parent < 0 || parent >= g_ns.count)
        return -1;

    child = g_ns.nodes[parent].first_child;
    while (child != 0xFFFF && hops++ <= g_ns.count && child >= 0 && child < ACPI_NS_MAX_NODES) {
        if (memcmp(g_ns.nodes[child].name, name, 4) == 0)
            return child;
        child = g_ns.nodes[child].next_sibling;
 ...
 */
static uint32_t aml_process_object(struct aml_parse_state *state, uint8_t opcode,
                                   uint8_t ext_opcode) {
        ...
        total_size = 1 + name_bytes;
        {
            ...
            if (dob < max_len) {
                ...
                if (dtype == AML_BYTE_PREFIX && dob + 2 <= max_len) {
                    ...
                    uint32_t buf_bytes;
                    uint32_t buf_len = aml_decode_pkg_length(aml, max_len, dob + 1, &buf_bytes);

                    dob += 1 + buf_bytes + buf_len;
                } else if (dtype == AML_PACKAGE_OP && dob + 2 <= max_len) {
                    uint32_t pkg_bytes2;
                    uint32_t pkg_len2 = aml_decode_pkg_length(aml, max_len, dob + 1, &pkg_bytes2);
    ...
    int node_idx =
        aml_add_node(name, node_type, parent, &aml[state->offset], total_size, state->ssdt_index);
...
static uint32_t aml_walk_terms(struct aml_parse_state *state, uint32_t max_offset) {
    ...
    case AML_BUFFER_OP: {
        uint32_t pkg_bytes;
        uint32_t pkg_len = aml_decode_pkg_length(aml, state->aml_len, o + 1, &pkg_bytes);
    ...
    default: {
        ...
        if (opcode >= 0x11 && opcode <= 0x2D) {
            ...
            if (o + 1 < state->aml_len) {
                ...
                pkg_len = aml_decode_pkg_length(aml, state->aml_len, o + 1, &pkg_bytes);
                if (pkg_len > 0)
                    return 1 + pkg_bytes + pkg_len;
            }
        }
...
int aml_build_namespace(void) {
    ...
    /* Create root node */
    {
        ...
        for (int i = 0; i < g_acpi_ssdt_count; i++) {
            if (g_acpi_ssdt_tables[i].aml_base == NULL || g_acpi_ssdt_tables[i].aml_length == 0)
                continue;
...
struct aml_ns_node *aml_ns_lookup(const char *path) {
    ...
    while (*path) {
        ...
        /* Bound the walk: a valid namespace has at most g_ns.count
         * nodes, so more hops means a corrupt/cyclic chain. */
        while (child != 0xFFFF && hops++ <= g_ns.count && child >= 0 && child < ACPI_NS_MAX_NODES) {
...
static const char *aml_ns_type_name(uint8_t type) {
    ...
    case AML_NS_DEVICE:
        return "DEVICE";
    case AML_NS_PROCESSOR:
        return "PROC";
    case AML_NS_POWERRESOURCE:
        return "POWR";
    case AML_NS_THERMAL_ZONE:
        return "THRM";
    case AML_NS_NAME:
        return "NAME";
    case AML_NS_METHOD:
...
void aml_dump_namespace(void) {
    ...
    for (int i = 0; i < g_ns.count; i++) {
        ...
        kprintf("  %-4d %-6s %-4.4s %-8d %-8d %s %u\n", i, aml_ns_type_name(node->type), node->name,
                node->parent, node->first_child, src_label, (unsigned int)node->from_ssdt);
    }
...
struct aml_object *aml_create_package(const struct aml_object *elements, uint32_t count) {
    struct aml_object *obj;
    ...
    obj->value.package.elements = (struct aml_object *)kmalloc(count * sizeof(struct aml_object));
    if (!obj->value.package.elements) {
        kfree(obj);
        return NULL;
    }

    memset(obj->value.package.elements, 0, count * sizeof(struct aml_object));
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        memcpy(&obj->value.package.elements[i], clone, sizeof(struct aml_object));
        kfree(clone);
    }
 ...
 */
static struct aml_object *aml_parse_buffer_from_aml(struct aml_exec_context *ctx) {
    ...
    /* Decode PkgLength */
    pkg_len = aml_decode_pkg_length(ctx->aml, ctx->aml_len, o + 1, &buf_bytes);
    ...
    if (ctx->aml[data_offset] == AML_BYTE_PREFIX && data_offset + 2 <= ctx->aml_len) {
        declared_size = ctx->aml[data_offset + 1];
        size_field_bytes = 2;
    } else if (ctx->aml[data_offset] == AML_WORD_PREFIX && data_offset + 3 <= ctx->aml_len) {
        declared_size =
            (uint32_t)ctx->aml[data_offset + 1] | ((uint32_t)ctx->aml[data_offset + 2] << 8);
        size_field_bytes = 3;
    } else if (ctx->aml[data_offset] == AML_DWORD_PREFIX && data_offset + 5 <= ctx->aml_len) {
    ...
    if (data_offset + size_field_bytes + declared_size > o + 1 + buf_bytes + pkg_len)
        return NULL;

    /* Read the raw byte data */
    obj = aml_create_buffer(&ctx->aml[data_offset + size_field_bytes], declared_size);
    if (obj)
        ctx->offset = o + 1 + buf_bytes + pkg_len;
 ...
 */
static struct aml_object *aml_parse_package_from_aml(struct aml_exec_context *ctx) {
    uint32_t o = ctx->offset;
    uint32_t pkg_bytes;
    uint32_t pkg_len;
    int is_var;
    ...
    if (ctx->aml[o] != AML_PACKAGE_OP && ctx->aml[o] != AML_VAR_PACKAGE_OP)
        return NULL;

    is_var = (ctx->aml[o] == AML_VAR_PACKAGE_OP);

    /* Decode PkgLength */
    pkg_len = aml_decode_pkg_length(ctx->aml, ctx->aml_len, o + 1, &pkg_bytes);
    if (pkg_len == 0 || pkg_len < 2)
        return NULL;
    ...
    elements = (struct aml_object *)kmalloc(num_elements * sizeof(struct aml_object));
    if (!elements) {
        ctx->pkg_depth--;
        return NULL;
    }
    ...
    for (uint32_t i = 0; i < num_elements && data_offset < element_end; i++) {
        ...
        case AML_WORD_PREFIX:
            if (data_offset + 3 <= ctx->aml_len) {
                data_offset++;
                elem = aml_create_integer(aml_read_le64(ctx->aml, ctx->aml_len, data_offset, 2));
                data_offset += 2;
            }
            break;
        case AML_DWORD_PREFIX:
            if (data_offset + 5 <= ctx->aml_len) {
                data_offset++;
                elem = aml_create_integer(aml_read_le64(ctx->aml, ctx->aml_len, data_offset, 4));
                data_offset += 4;
            }
            break;
        case AML_QWORD_PREFIX:
            if (data_offset + 9 <= ctx->aml_len) {
                data_offset++;
                elem = aml_create_integer(aml_read_le64(ctx->aml, ctx->aml_len, data_offset, 8));
                data_offset += 8;
            }
            break;
        case AML_STRING_PREFIX: {
...
static struct aml_object *aml_clone_object(const struct aml_object *src) {
    ...
    case AML_OBJ_STRING:
        if (src->value.string.ptr) {
            dst->value.string.ptr = (char *)kmalloc(src->value.string.len + 1);
            if (!dst->value.string.ptr) {
                kfree(dst);
                return NULL;
            }
            memcpy(dst->value.string.ptr, src->value.string.ptr, src->value.string.len + 1);
    ...
    case AML_OBJ_BUFFER:
        if (src->value.buffer.length > 0 && src->value.buffer.data) {
            dst->value.buffer.data = (uint8_t *)kmalloc(src->value.buffer.length);
            if (!dst->value.buffer.data) {
                kfree(dst);
                return NULL;
            }
            memcpy(dst->value.buffer.data, src->value.buffer.data, src->value.buffer.length);
            dst->value.buffer.length = src->value.buffer.length;
        }
        break;

    case AML_OBJ_PACKAGE:
        if (src->value.package.count > 0 && src->value.package.elements) {
            uint32_t count = src->value.package.count;

            dst->value.package.elements =
                (struct aml_object *)kmalloc(count * sizeof(struct aml_object));
            if (!dst->value.package.elements) {
                kfree(dst);
                return NULL;
            }
            memset(dst->value.package.elements, 0, count * sizeof(struct aml_object));
            dst->value.package.count = count;

            for (uint32_t i = 0; i < count; i++) {
                struct aml_object *elem;

                elem = aml_clone_object(&src->value.package.elements[i]);
                if (!elem) {
                    for (uint32_t j = 0; j < i; j++)
                        aml_free_object(&dst->value.package.elements[j]);
                    kfree(dst->value.package.elements);
                    kfree(dst);
                    return NULL;
                }
                memcpy(&dst->value.package.elements[i], elem, sizeof(struct aml_object));
                kfree(elem);
            }
        }
        break;
...
static int aml_is_name_string_start(uint8_t byte) {
    if (byte == AML_ROOT_PREFIX || byte == AML_PARENT_PREFIX || byte == AML_DUAL_NAME_PREFIX ||
        byte == AML_MULTI_NAME_PREFIX)
        return 1;
 ...
 */
static int aml_store_local_name(struct aml_exec_context *ctx, const char name[4],
 ...
 */
static struct aml_object *aml_resolve_namestring_value(struct aml_exec_context *ctx,
                                                       uint32_t offset, int *scope_used) {
        ...
        if (prefix == AML_DUAL_NAME_PREFIX || prefix == AML_MULTI_NAME_PREFIX) {
            ...
            if (prefix == AML_DUAL_NAME_PREFIX) {
                ...
                if (o + 4 > max_len)
                    break;
                uint32_t so = o;
                int c = aml_find_child_safe(current_scope, (const char *)&aml[so]);
                if (c >= 0)
                    current_scope = c;
                o += 4;
                ...
                for (uint8_t i = 0; i < count - 1; i++) {
                    int c = aml_find_child_safe(current_scope, (const char *)&aml[o]);
                    if (c >= 0)
                        current_scope = c;
                    o += 4;
                }
    ...
    if (current_scope >= 0 && current_scope < g_ns.count) {
        int found = aml_find_child_safe(current_scope, name_seg);

        if (found >= 0 && g_ns.nodes[found].type == AML_NS_NAME) {
 ...
 */
static struct aml_object *aml_eval_data_ref_object(struct aml_exec_context *ctx) {
    uint8_t op;
    struct aml_object *obj = NULL;
    ...
    case AML_WORD_PREFIX:
        if (ctx->offset + 3 <= ctx->aml_len) {
            ctx->offset++;
            obj = aml_create_integer(aml_read_le64(ctx->aml, ctx->aml_len, ctx->offset, 2));
            ctx->offset += 2;
        }
        break;
    ...
    case AML_DWORD_PREFIX:
        if (ctx->offset + 5 <= ctx->aml_len) {
            ctx->offset++;
            obj = aml_create_integer(aml_read_le64(ctx->aml, ctx->aml_len, ctx->offset, 4));
            ctx->offset += 4;
        }
        break;
    ...
    case AML_QWORD_PREFIX:
        if (ctx->offset + 9 <= ctx->aml_len) {
            ctx->offset++;
            obj = aml_create_integer(aml_read_le64(ctx->aml, ctx->aml_len, ctx->offset, 8));
            ctx->offset += 8;
        }
        break;
    ...
    default:
        ...
        if (aml_is_name_string_start(op)) {
            ...
            uint32_t name_bytes = aml_skip_name_string(ctx->aml, ctx->aml_len, ctx->offset);

            if (name_bytes > 0) {
                /* Resolve the named value */
                val = aml_resolve_namestring_value(ctx, saved_offset, NULL);
                if (val) {
...
static int aml_exec_name_op(struct aml_exec_context *ctx) {
    ...
    name_bytes = aml_read_last_nameseg(ctx->aml, ctx->aml_len, ctx->offset, name_seg);
    if (name_bytes == 0)
        return -1;

    /* Skip the full NameString */
    {
        uint32_t skip = aml_skip_name_string(ctx->aml, ctx->aml_len, ctx->offset);
    ...
    if (ctx->local_name_count < AML_MAX_LOCAL_NAMES || aml_find_local_name(ctx, name_seg) >= 0) {
    ...
    if (!stored) {
        ...
        if (scope >= 0 && scope < g_ns.count)
            found = aml_find_child_safe(scope, name_seg);
...
static int aml_exec_store(struct aml_exec_context *ctx) {
    ...
    {
        uint8_t op = (ctx->offset < ctx->aml_len) ? ctx->aml[ctx->offset] : 0;
        ...
        case AML_WORD_PREFIX:
            ...
            operand = &operand_buf;
            operand->type = AML_OBJ_INTEGER;
            operand->value.integer = aml_read_le64(ctx->aml, ctx->aml_len, ctx->offset, 2);
            ctx->offset += 2;
            break;
        ...
        case AML_DWORD_PREFIX:
            ...
            operand = &operand_buf;
            operand->type = AML_OBJ_INTEGER;
            operand->value.integer = aml_read_le64(ctx->aml, ctx->aml_len, ctx->offset, 4);
            ctx->offset += 4;
            break;
        ...
        case AML_QWORD_PREFIX:
            ...
            operand = &operand_buf;
            operand->type = AML_OBJ_INTEGER;
            operand->value.integer = aml_read_le64(ctx->aml, ctx->aml_len, ctx->offset, 8);
            ctx->offset += 8;
            break;
        ...
        case AML_LOCAL7: {
            ...
            if (ctx->locals[idx]) {
                ...
                if (!clone)
                    goto done;
                memcpy(&operand_buf, clone, sizeof(operand_buf));
                kfree(clone);
            } else {
                operand = &operand_buf;
                operand->type = AML_OBJ_INTEGER;
    ...
    /* Parse the Target (destination: where to store) */
    {
        ...
        default:
            if (aml_is_name_string_start(op)) {
                ...
                name_bytes = aml_skip_name_string(ctx->aml, ctx->aml_len, ctx->offset);
                if (name_bytes == 0)
                    goto done;
                ...
                /* Read name_seg */
                aml_read_last_nameseg(ctx->aml, ctx->aml_len, name_offset, name_seg);

                /* Check method-local names first */
                {
                    int li = aml_find_local_name(ctx, name_seg);
                ...
                if (operand) {
                    val = aml_clone_object(operand);
                    if (val) {
                        aml_store_local_name(ctx, name_seg, val);
                        ret = 0;
                        goto done;
                    }
                }
...
static int aml_exec_return(struct aml_exec_context *ctx) {
    ...
    case AML_WORD_PREFIX:
        ...
        ctx->offset++;
        ctx->return_value->type = AML_OBJ_INTEGER;
        ctx->return_value->value.integer = aml_read_le64(ctx->aml, ctx->aml_len, ctx->offset, 2);
        ctx->offset += 2;
        break;
    ...
    case AML_DWORD_PREFIX:
        ...
        ctx->offset++;
        ctx->return_value->type = AML_OBJ_INTEGER;
        ctx->return_value->value.integer = aml_read_le64(ctx->aml, ctx->aml_len, ctx->offset, 4);
        ctx->offset += 4;
        break;
    ...
    case AML_QWORD_PREFIX:
        ...
        ctx->offset++;
        ctx->return_value->type = AML_OBJ_INTEGER;
        ctx->return_value->value.integer = aml_read_le64(ctx->aml, ctx->aml_len, ctx->offset, 8);
        ctx->offset += 8;
        break;
    ...
    case AML_STRING_PREFIX: {
        ...
        ctx->offset++;
        start = ctx->offset;
        while (ctx->offset < ctx->aml_len && ctx->aml[ctx->offset] != 0)
            ctx->offset++;
        if (ctx->offset >= ctx->aml_len)
            return -1;

        ctx->return_value->type = AML_OBJ_STRING;
        ctx->return_value->value.string.len = ctx->offset - start;
        ctx->return_value->value.string.ptr =
            (char *)kmalloc(ctx->return_value->value.string.len + 1);
        if (!ctx->return_value->value.string.ptr)
            return -1;
        memcpy(ctx->return_value->value.string.ptr, &ctx->aml[start],
               ctx->return_value->value.string.len);
        ctx->return_value->value.string.ptr[ctx->return_value->value.string.len] = '\0';
    ...
    case AML_LOCAL7: {
        ...
        if (ctx->locals[idx]) {
            ...
            clone = aml_clone_object(ctx->locals[idx]);
            if (clone) {
                memcpy(ctx->return_value, clone, sizeof(*clone));
                kfree(clone);
            }
        } else {
            ctx->return_value->type = AML_OBJ_INTEGER;
    ...
    case AML_BUFFER_OP: {
        ...
        buf_obj = aml_parse_buffer_from_aml(ctx);
        if (buf_obj) {
            memcpy(ctx->return_value, buf_obj, sizeof(*buf_obj));
            kfree(buf_obj);
        } else {
            ctx->return_value->type = AML_OBJ_INTEGER;
            ctx->return_value->value.integer = 0;
    ...
    case AML_VAR_PACKAGE_OP: {
        ...
        pkg_obj = aml_parse_package_from_aml(ctx);
        if (pkg_obj) {
            memcpy(ctx->return_value, pkg_obj, sizeof(*pkg_obj));
            kfree(pkg_obj);
        } else {
            ctx->return_value->type = AML_OBJ_INTEGER;
            ctx->return_value->value.integer = 0;
    ...
    default:
        ...
        if (aml_is_name_string_start(op)) {
            ...
            if (name_bytes > 0) {
                ctx->offset += name_bytes;
                val = aml_resolve_namestring_value(ctx, name_offset, NULL);
                if (val) {
                    ctx->return_value = &ctx->own_return;
                    memset(ctx->return_value, 0, sizeof(*ctx->return_value));
                    if (val->type == AML_OBJ_INTEGER) {
                        ctx->return_value->type = AML_OBJ_INTEGER;
                        ctx->return_value->value.integer = val->value.integer;
                    } else {
                        struct aml_object *clone;
...
static uint64_t aml_read_arith_operand(struct aml_exec_context *ctx) {
    ...
    case AML_WORD_PREFIX:
        if (ctx->offset + 2 > ctx->aml_len)
            goto error;
        {
            uint64_t v = aml_read_le64(ctx->aml, ctx->aml_len, ctx->offset, 2);
    ...
    case AML_DWORD_PREFIX:
        if (ctx->offset + 4 > ctx->aml_len)
            goto error;
        {
            uint64_t v = aml_read_le64(ctx->aml, ctx->aml_len, ctx->offset, 4);
    ...
    case AML_QWORD_PREFIX:
        if (ctx->offset + 8 > ctx->aml_len)
            goto error;
        {
            uint64_t v = aml_read_le64(ctx->aml, ctx->aml_len, ctx->offset, 8);
    ...
    case AML_LOCAL7: {
        ...
        if (ctx->locals[idx] && ctx->locals[idx]->type == AML_OBJ_INTEGER)
    ...
    case AML_ARG6: {
        ...
        if (ctx->args[idx] && ctx->args[idx]->type == AML_OBJ_INTEGER)
    ...
    default:
        ...
        if (aml_is_name_string_start(op)) {
            ...
            {
                uint32_t skip = aml_skip_name_string(ctx->aml, ctx->aml_len, ctx->offset - 1);
 ...
 */
static int aml_write_arith_result(struct aml_exec_context *ctx, uint64_t value) {
    uint8_t op;
...
static int aml_exec_arith(struct aml_exec_context *ctx, uint8_t opcode) {
    ...
    case AML_DIVIDE_OP: {
        ...
        /* Write remainder */
        if (aml_write_arith_result(ctx, operand1 % operand2) < 0)
            return -1;
        /* Write quotient */
        if (aml_write_arith_result(ctx, operand1 / operand2) < 0)
            return -1;
...
static int aml_exec_if(struct aml_exec_context *ctx) {
    ...
    pkg_len = aml_decode_pkg_length(ctx->aml, ctx->aml_len, ctx->offset, &pkg_bytes);
    if (pkg_len == 0)
        return -1;
    ...
    if (predicate_true) {
        ...
        while (walk_offset < body_end) {
            ...
            if (ctx->aml[walk_offset] == AML_ELSE_OP) {
                ...
                else_pkg_len2 =
                    aml_decode_pkg_length(ctx->aml, ctx->aml_len, walk_offset, &else_pkg_bytes2);
                if (else_pkg_len2 > 0)
                    walk_offset += else_pkg_bytes2 + else_pkg_len2;
            ...
            if (ctx->break_flag || ctx->has_returned || ctx->error)
                break;
        }
    } else {
            ...
            if (ctx->aml[walk_offset] == AML_ELSE_OP) {
                ...
                while (else_body < else_end) {
                    ctx->offset = else_body;
                    {
                        ...
                        if (consumed == 0 || ctx->error || ctx->has_returned || ctx->break_flag)
                            break;
...
static int aml_exec_while(struct aml_exec_context *ctx) {
    ...
    while (1) {
        ...
        while (ctx->offset < body_end && !ctx->has_returned && !ctx->error && !ctx->break_flag) {
            uint32_t consumed = aml_exec_one_term(ctx);
...
static uint32_t aml_exec_one_term(struct aml_exec_context *ctx) {
    ...
    case AML_ELSE_OP: {
        ...
        else_pkg_len = aml_decode_pkg_length(ctx->aml, ctx->aml_len, ctx->offset, &else_pkg_bytes);
        if (else_pkg_len > 0)
    ...
    default:
        ...
        if (opcode == 0x90 || opcode == 0x91 || opcode == 0x92 || opcode == 0x93 ||
            opcode == 0x94) {
            ctx->offset = o + 3;
            return 3;
        }
...
static int aml_exec_method_body(struct aml_exec_context *ctx) {
    while (ctx->offset < ctx->aml_len && !ctx->has_returned && !ctx->error) {
        uint32_t consumed = aml_exec_one_term(ctx);
...
static void aml_exec_context_cleanup(struct aml_exec_context *ctx) {
    ...
    if (ctx->return_value == &ctx->own_return && ctx->return_value->from_heap) {
        if (ctx->return_value->type == AML_OBJ_STRING && ctx->return_value->value.string.ptr) {
            ...
            for (uint32_t i = 0; i < ctx->return_value->value.package.count; i++)
                aml_free_object(&ctx->return_value->value.package.elements[i]);
...
struct aml_object *aml_evaluate_method(const char *path, struct aml_object *args[], int num_args) {
    ...
    if (num_args > (int)method_arg_count) {
        kprintf("[AML] Method '%s' declares %u args, got %d "
                "(continuing anyway)\n",
                path ? path : "(null)", (unsigned int)method_arg_count, num_args);
    }
    if (num_args < (int)method_arg_count) {
    ...
    kprintf("[AML] Evaluating method '%s' (%u AML bytes, %u args)\n", path ? path : "(null)",
            (unsigned int)ctx.aml_len, (unsigned int)method_arg_count);
...
struct aml_object *aml_create_opregion(uint8_t space_id, uint64_t region_offset,
...
int aml_opregion_register_handler(uint8_t space_id, aml_opregion_handler_t handler, void *context) {
    int i;
...
int aml_opregion_access(const struct aml_opregion_info *region, uint64_t offset, uint8_t *buf,
    ...
    for (i = 0; i < g_opregion_handler_count; i++) {
        if (g_opregion_handlers[i].space_id == region->space_id) {
            ...
            access.write = write;
            return g_opregion_handlers[i].handler(region, &access, g_opregion_handlers[i].context);
        }
    }
...
static int aml_smbus_handler(const struct aml_opregion_info *region,
                             struct aml_opregion_access *access, void *context) {
    uint8_t slave_addr;
    uint8_t cmd;
    uint32_t remaining;
    uint32_t buf_off;
    ...
    while (remaining > 0) {
        if (remaining >= 2 && (access->offset & 1) == 0) {
            ...
            if (access->write) {
                uint16_t wval =
                    (uint16_t)access->buf[buf_off] | ((uint16_t)access->buf[buf_off + 1] << 8);
...
static void aml_opregion_init(void) {
    ...
    ret = aml_opregion_register_handler(AML_OPREGION_SMBUS, aml_smbus_handler, NULL);
    if (ret < 0) {
