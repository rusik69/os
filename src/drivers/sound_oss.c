...
static int oss_ensure_streams(void) {
    ...
    if (!g_playback) {
        ...
        s = (struct sound_pcm_stream *)kmalloc(sizeof(struct sound_pcm_stream));
        if (!s) {
            ret = -ENOMEM;
            goto out;
        }

        ret = sound_pcm_init_stream(s, SOUND_PCM_PLAYBACK, (uint32_t)g_frag_size,
    ...
    if (!g_capture) {
        ...
        ret = sound_pcm_init_stream(s, SOUND_PCM_CAPTURE, (uint32_t)g_frag_size,
                                    (uint32_t)g_frag_count, NULL);
        if (ret < 0) {
            kfree(s);
            goto out;
...
static void dma_transfer_simulate(void) {
    ...
    uint32_t nread = frag_size > DMA_OUT_BUF_SIZE ? DMA_OUT_BUF_SIZE : frag_size;
...
static int dsp_read(void *priv, void *buf, uint32_t max_size, uint32_t *out_size) {
    ...
    int captured = ac97_capture_read((int16_t *)buf, bytes_to_capture, (uint32_t)g_sample_rate);
...
static int oss_read_arg32(uint64_t arg, uint32_t *out) {
    return copy_from_user(out, arg, sizeof(uint32_t)) < 0 ? -EFAULT : 0;
}
...
static int oss_write_arg32(uint64_t arg, uint32_t val) {
    return copy_to_user(arg, &val, sizeof(uint32_t)) < 0 ? -EFAULT : 0;
}
 ...
 * Returns 0 on success, negative errno on failure.
 */
static int oss_readwrite_arg(uint64_t arg, uint32_t cmd, uint32_t *in_val, uint32_t *out_val) {
...
int sound_oss_ioctl(int cmd, uint64_t arg) {
    ...
    case SNDCTL_DSP_SPEED: {
        ...
        if (g_playback) {
            sound_pcm_reset(g_playback);
            sound_pcm_init_stream(g_playback, SOUND_PCM_PLAYBACK, (uint32_t)g_frag_size,
                                  (uint32_t)g_frag_count, NULL);
        }
        if (g_capture) {
            sound_pcm_reset(g_capture);
            sound_pcm_init_stream(g_capture, SOUND_PCM_CAPTURE, (uint32_t)g_frag_size,
                                  (uint32_t)g_frag_count, NULL);
        }
    ...
    case SNDCTL_DSP_SETFRAGMENT: {
        ...
        if (frag_shift < 4)
            frag_shift = 4; /* min 16 bytes */
        if (frag_shift > 15)
            frag_shift = 15; /* max 32K */
        if (num_frags < 2)
            num_frags = 2;
        if (num_frags > 64)
            num_frags = 64;
        ...
        uint32_t actual_fragspec =
            ((uint32_t)(31 - __builtin_clz((uint32_t)g_frag_size)) & 0xFFFF) |
    ...
    case SNDCTL_DSP_GETOSPACE: {
        ...
        if (g_playback)
            avail_bytes = (int)sound_pcm_get_avail_write(g_playback);
        else
            avail_bytes = g_frag_size * g_frag_count;

        info.fragments = g_frag_size > 0 ? avail_bytes / g_frag_size : 0;
    ...
    case SNDCTL_DSP_GETISPACE: {
        ...
        if (g_capture)
            avail_bytes = (int)sound_pcm_get_avail_write(g_capture);
        else
            avail_bytes = g_frag_size * g_frag_count;
    ...
    case SNDCTL_DSP_GETOPTR: {
        ...
        info.blocks = g_frag_size > 0 ? (int)(total_bytes / (uint32_t)g_frag_size) : 0;
        info.ptr = g_playback ? (int)(g_playback->hw_ptr & (g_playback->buf_mask)) : 0;
    ...
    case SNDCTL_DSP_GETIPTR: {
        ...
        info.bytes = (int)total_bytes;
        info.blocks = g_frag_size > 0 ? (int)(total_bytes / (uint32_t)g_frag_size) : 0;
        info.ptr = g_capture ? (int)(g_capture->app_ptr & (g_capture->buf_mask)) : 0;
    ...
    case SNDCTL_DSP_GETRECORD_GAIN: {
        ...
        val = (uint32_t)g_record_gain_left | ((uint32_t)g_record_gain_right << 8);
        if (g_record_mute)
            val |= 0x8000;
    ...
    case SNDCTL_DSP_GETFMTS: {
        uint32_t fmts = AFMT_BIT_U8 | AFMT_BIT_S16_LE | AFMT_BIT_S16_BE;
    ...
    case SNDCTL_DSP_GETPLAYVOL: {
        uint32_t vol = (uint32_t)g_dsp_play_volume | ((uint32_t)g_dsp_play_volume << 8);
    ...
    case SNDCTL_DSP_GETRECVOL: {
        uint32_t vol = (uint32_t)g_dsp_rec_volume | ((uint32_t)g_dsp_rec_volume << 8);
        return oss_write_arg32(arg, vol);
    }
    ...
    case SNDCTL_DSP_PROFILE: {
        ...
        return oss_write_arg32(arg, (uint32_t)g_dsp_profile);
    }
    ...
    case SOUND_MIXER_WRITE_VOLUME: {
        ...
        if (ret < 0)
            return ret;
        ret = sound_mixer_write(SOUND_MIXER_MASTER, (uint16_t)val);
        if (ret < 0)
            return ret;
    ...
    case SOUND_MIXER_WRITE_MUTE: {
        ...
        if (ret < 0)
            return ret;
        ret = sound_mixer_set_mute(SOUND_MIXER_MASTER, (int)val ? 1 : 0);
        if (ret < 0)
            return ret;
    ...
    case SOUND_MIXER_WRITE_RECSRC: {
        ...
        if (ret < 0)
            return ret;
        for (int i = 0; i < SOUND_MIXER_COUNT; i++) {
            ret = sound_mixer_set_recsel((enum sound_mixer_channel)i, (mask & (1u << i)) ? 1 : 0);
            if (ret < 0)
                return ret;
        }
