...
static int sysctl_write_sched_eevdf(const char *buf, int len) {
    if (len > 0 && buf[0] == '1')
        sched_eevdf_enabled = 1;
    else
        sched_eevdf_enabled = 0;
...
static int sysctl_write_sched_latency(const char *buf, int len) {
    ...
    if (v < 100000ULL)
        v = 100000ULL; /* floor: 100 µs */
    if (v > 1000000000ULL)
        v = 1000000000ULL; /* ceil:  1 s   */
    sched_latency_ns = v;
...
static int sysctl_write_sched_min_granularity(const char *buf, int len) {
    ...
    for (int i = 0; i < len && buf[i] >= '0' && buf[i] <= '9'; i++)
        v = v * 10 + (uint64_t)(buf[i] - '0');
    if (v < 50000ULL)
        v = 50000ULL; /* floor: 50 µs  */
    if (v > 100000000ULL)
        v = 100000000ULL; /* ceil: 100 ms  */
...
static inline int weight_to_nice(int weight) {
    ...
    if (weight <= sched_prio_to_weight[39])
        return NICE_MAX;
    for (int i = 0; i < 39; i++) {
        if (weight >= sched_prio_to_weight[i + 1] && weight <= sched_prio_to_weight[i])
            return NICE_MIN + i;
    }
    return 0;
}
...
void __init scheduler_init(void) {
    ...
    sysctl_register("sched_latency_ns", sysctl_read_sched_latency, sysctl_write_sched_latency);
    sysctl_register("sched_min_granularity_ns", sysctl_read_sched_min_granularity,
                    sysctl_write_sched_min_granularity);
    /* Register EEVDF enable/disable switch (exposed as /sys/kernel/sched_eevdf) */
    sysctl_register("sched_eevdf", sysctl_read_sched_eevdf, sysctl_write_sched_eevdf);
}
...
static void scheduler_add_locked(struct process *proc) {
    ...
    if (home >= 0 && home < NUMA_MAX_NODES) {
        ...
        if (!numa_cpu_is_on_node((int)cpu_id, home)) {
            ...
            if (home_cpu >= 0 && home_cpu < smp_cpu_count && cpuhp_is_online(home_cpu)) {
...
void schedule(void) {
    ...
    if (ci->preempt_count > 0) {
        ...
        kprintf("    CPU=%d, current_process=%s (pid=%u)\n", ci->cpu_id,
                ci->current_process && ci->current_process->name ? ci->current_process->name : "?",
    ...
    if (current == next) {
        ...
        if (info_guard)
            info_guard->current_kernel_rsp = next->stack_top;
    ...
    if (info)
        info->current_kernel_rsp = next->stack_top;
...
void scheduler_tick(int was_user) {
    ...
    /* Wake blocked processes whose sleep_until expired.  Without this,
    ...
    /* ── RLIMIT_CPU enforcement ──────────────────────────────── */
    {
        ...
        if (cpu_sec != RLIM_INFINITY && cpu_sec > 0) {
            ...
            if (total_sec > cpu_sec) {
                ...
                } else if (timer_get_ticks() - cur->cpu_limit_warned_tick >= (uint64_t)TIMER_FREQ) {
...
void scheduler_stats_inc_ctx_switch(void) {
    sched_stats_data.context_switches++;
}
void scheduler_stats_inc_preempt(void) {
    sched_stats_data.preemptions++;
}
...
int sched_autogroup_get(int session_id) {
    ...
    if (autogroups[target].id == session_id)
        return target;

    /* Find existing group */
    for (int i = 0; i < SCHED_AUTOGROUP_MAX; i++) {
        if (autogroups[i].id == session_id)
            return i;
    }
...
uint64_t sched_autogroup_max_vruntime(int group_id) {
    ...
    struct process *table = process_get_table();
    for (int i = 0; i < PROCESS_MAX; i++) {
        if (table[i].state != PROCESS_UNUSED && table[i].sched_autogroup_id == group_id &&
...
static inline int sched_class_rank(uint8_t policy) {
    if (sched_rt_policy(policy))
        return 1;
    if (sched_cfs_policy(policy))
        return 2;
    if (sched_idle_policy(policy))
        return 3;
...
static void scheduler_wakeup_locked(struct process *proc) {
    ...
    /* If the task's vruntime is far behind, cap it (CFS path) */
    if (proc->vruntime + CFS_MIN_VRUNTIME_OFFSET < min_vr) {
        proc->vruntime = min_vr - CFS_MIN_VRUNTIME_OFFSET;
    } else if (proc->sched_boost_on_wake) {
        /* Wake-boost (CFS path): reset the vruntime below the minimum. */
        proc->vruntime = min_vr > CFS_MIN_VRUNTIME_OFFSET ? min_vr - CFS_MIN_VRUNTIME_OFFSET : 0;
    }

    /* Add to the runqueue (caller holds sched_lock, use locked variant) */
    scheduler_add_locked(proc);

    /* Wake-boost (EEVDF path): scheduler_add_locked → eevdf_enqueue has
     * just re-initialized lag=0 and deadline=now+slice, which would make
     * this task tie with every fresh child on the eligible-deadline pick
     * and lose (linear scan order).  Forcing the deadline to 0 makes the
     * eligible deadline 0, so the netd wins the pick whenever it is
     * runnable — it blocks again after each 2-tick poll, so it cannot
     * hog the CPU.  Without this, EEVDF starves the netd once init's
     * fork loop floods the runqueue and no packet ever gets processed. */
    if (proc->sched_boost_on_wake)
