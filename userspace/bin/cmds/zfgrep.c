/* zfgrep.c — Fgrep compressed files (pipe through decompress then grep -F) */
#include "unistd.h"
#include "string.h"
#include "stdio.h"
#include "stdlib.h"
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 2; i < argc; i++) {
        ...
        if (!gz) {
            int pid = fork();
            if (pid == 0) {
                char *gr_argv[] = { (char *)"grep", (char *)"-F", (char *)"-H",
                                    (char *)pattern, (char *)argv[i], NULL };
                char *env[] = { NULL, NULL };
                execve("/bin/grep", gr_argv, env);
                printf("zfgrep: grep not found\n");
                exit(1);
            }
            int status;
            ...
            if (pid2 == 0) {
                ...
                dup2(pipe_fds[0], 0);
                close(pipe_fds[0]);
                char *gr_argv2[] = { (char *)"grep", (char *)"-F", (char *)"-H", (char *)pattern, NULL };
                char *env2[] = { NULL, NULL };
