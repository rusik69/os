/* sha256sum.c — compute SHA-256 message digest */
#include "unistd.h"
#include "stdio.h"
#include "string.h"
#include "stdlib.h"
...
struct sha256_ctx{uint32_t h[8];uint64_t nbytes;};

static void sha256_init(struct sha256_ctx*c){
    c->h[0]=0x6a09e667;c->h[1]=0xbb67ae85;c->h[2]=0x3c6ef372;c->h[3]=0xa54ff53a;
    c->h[4]=0x510e527f;c->h[5]=0x9b05688c;c->h[6]=0x1f83d9ab;c->h[7]=0x5be0cd19;c->nbytes=0;
}
static void sha256_transform(uint32_t h[8],const unsigned char*block){
...
static void sha256_update(struct sha256_ctx*c,const unsigned char*data,unsigned long len){
    ...
    while(len>0){unsigned long chunk=(64-off<len)?64-off:len;if(chunk<64){memcpy((unsigned char*)c->h+off,data,chunk);off+=chunk;data+=chunk;len-=chunk;}
        else{unsigned char buf[64];memcpy(buf,data,64);sha256_transform(c->h,buf);off=0;data+=64;len-=64;}}
}
static void sha256_final(struct sha256_ctx*c,unsigned char*out){
...
int main(int argc,char*argv[]){
    ...
    unsigned char buf[8192];struct sha256_ctx ctx;unsigned char digest[32];
    if(fn){int fd=open(fn,O_RDONLY,0);
        if(fd<0){printf("sha256sum: %s: No such file\n",fn);return 1;}
        sha256_init(&ctx);int n;
        while((n=read(fd,buf,sizeof(buf)))>0)sha256_update(&ctx,buf,n);
        close(fd);sha256_final(&ctx,digest);
    }else{sha256_init(&ctx);int n;while((n=read(0,buf,sizeof(buf)))>0)sha256_update(&ctx,buf,n);sha256_final(&ctx,digest);fn="-";}
    for(int i=0;i<32;i++){printf("%02x",digest[i]);}printf("  %s\n",fn);return 0;
}
