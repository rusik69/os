/* zegrep.c — Grep compressed files with extended regex (pipe through decompress then grep -E) */
#include "unistd.h"
#include "string.h"
#include "stdio.h"
#include "stdlib.h"
...
int main(int argc, char *argv[]) {
    if (argc < 3) {
        printf("usage: zegrep <pattern> <file...>\n");
        return 1;
    }
    ...
    for (int i = 2; i < argc; i++) {
        ...
        if (!gz) {
            int pid = fork();
            if (pid == 0) {
                char *gr_argv[] = { (char *)"grep", (char *)"-E", (char *)"-H",
                                    (char *)pattern, (char *)argv[i], NULL };
                char *env[] = { NULL, NULL };
                execve("/bin/grep", gr_argv, env);
                printf("zegrep: grep not found\n");
                exit(1);
            }
            int status;
            ...
            if (pid2 == 0) {
                ...
                dup2(pipe_fds[0], 0);
                close(pipe_fds[0]);
                char *gr_argv[] = { (char *)"grep", (char *)"-E", (char *)"-H", (char *)pattern, NULL };
                char *env2[] = { NULL, NULL };
                execve("/bin/grep", gr_argv, env2);
                char line[4096];
