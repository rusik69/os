...
static void page_next(gui_widget_t *btn) {
    (void)btn;
    if (g_page < 3)
        g_page++;
    else
...
static void page_prev(gui_widget_t *btn) {
    (void)btn;
    if (g_page > 1)
        g_page--;
    else
...
static void draw_taskbar(int32_t mx, int32_t my) {
    ...
    tb_btn_t btns_p3[] = {
        ...
        {"NOTE", 428, 60, launch_notepad},
        {"<PAGE", 490, 44, page_prev},
        {"TERM", 540, 44, launch_terminal},
        {"FMGR", 586, 44, launch_file_manager},
        {"SETS", 632, 44, launch_settings},
        {"IMG", 682, 44, launch_image_viewer},
        {"MUSIC", 732, 48, launch_music_player},
        {"CLK2", 800, 44, launch_clock},
    ...
    if (g_page == 1) {
        btns = btns_p1;
        n = sizeof(btns_p1) / sizeof(btns_p1[0]);
    } else if (g_page == 2) {
        btns = btns_p2;
        n = sizeof(btns_p2) / sizeof(btns_p2[0]);
    } else {
        btns = btns_p3;
        n = sizeof(btns_p3) / sizeof(btns_p3[0]);
    }

    for (int i = 0; i < n; i++) {
        gui_color_t c =
            (btns[i].click == exit_gui) ? GUI_COLOR(100, 50, 50) : GUI_COLOR(70, 70, 100);
        if (mx >= btns[i].x && mx < btns[i].x + btns[i].w && my >= TASKBAR_Y &&
            my < TASKBAR_Y + TASKBAR_H)
...
static gui_window_t *find_window_at(int32_t x, int32_t y) {
    for (gui_window_t *w = gui_get_window_list(); w; w = gui_window_next(w)) {
        if (gui_window_contains_point(w, x, y))
            return w;
    }
    return NULL;
}
...
static void build_desktop(void) {
    ...
    gui_widget_t *lbl = gui_label_create(
        lr, "OS Desktop v2.0  |  47 GUI apps  |  28 drawing primitives  |  15+ widgets");
    if (lbl) {
...
void gui_shell_run(void) {
    ...
    while (g_running) {
        ...
        if ((buttons & 1) && g_drag_win) {
            ...
            if (ny < 0)
                ny = 0;
            if (nx + (int32_t)r.w > 1024)
                nx = 1024 - (int32_t)r.w;
            if (ny + (int32_t)r.h > TASKBAR_Y)
                ny = TASKBAR_Y - (int32_t)r.h;
            r.x = nx;
            r.y = ny;
        ...
        if (left_pressed) {
            if (py >= TASKBAR_Y) {
                tb_btn_t btns_p1[] = {
                    {"", 2, 44, launch_draw},
                    {"", 48, 50, launch_widgets},
                    {"", 100, 52, launch_colors},
                    {"", 154, 42, launch_gradient},
                    {"", 198, 54, launch_shapes},
                    {"", 254, 50, launch_checker},
                    {"", 306, 42, launch_info},
                    {"", 350, 46, open_file_win},
                    {"", 398, 52, launch_mandelbrot},
                    {"", 452, 42, launch_calc},
                    {"", 496, 38, launch_rgb_mixer},
                    {"", 536, 46, launch_analog_clock},
                    {"", 584, 52, launch_digital_clock},
                    {"", 638, 46, launch_paint},
                    {"", 686, 42, launch_minesweeper},
                    {"", 730, 46, launch_snake},
                    {"", 778, 50, launch_tetris},
                    {"", 830, 52, launch_lissajous},
                    {"", 884, 44, launch_starfield},
                    {"", 930, 42, page_next},
                    {"", 974, 44, exit_gui},
                };
                tb_btn_t btns_p2[] = {
                    {"", 2, 38, launch_fire},           {"", 42, 50, launch_plasma},
                    {"", 94, 42, launch_particles},     {"", 138, 42, launch_sort_viz},
                    {"", 182, 42, launch_wave},         {"", 226, 46, launch_noise},
                    {"", 274, 42, launch_heatmap},      {"", 318, 42, launch_fractal_tree},
                    {"", 362, 42, launch_sierpinski},   {"", 406, 42, launch_cellular},
                    {"", 450, 46, launch_moire},        {"", 498, 50, launch_tunnel},
                    {"", 550, 42, launch_metaballs},    {"", 594, 42, launch_snow},
                    {"", 638, 42, launch_gravity},      {"", 682, 42, launch_rotozoom},
                    {"", 726, 52, launch_kaleidoscope}, {"", 780, 42, launch_eyes},
                    {"", 824, 42, launch_bars},         {"", 868, 42, launch_bouncing_ball},
                    {"", 912, 50, launch_spiral},       {"", 964, 42, page_prev},
                    {"", 1008, 44, exit_gui},
                };
                tb_btn_t btns_p3[] = {
                    ...
                    {"", 632, 44, launch_settings},   {"", 682, 44, launch_image_viewer},
                    {"", 732, 44, exit_gui},
                };
                tb_btn_t *b;
                int nb;
                if (g_page == 1) {
                    b = btns_p1;
                    nb = sizeof(btns_p1) / sizeof(btns_p1[0]);
                } else if (g_page == 2) {
                    b = btns_p2;
                    nb = sizeof(btns_p2) / sizeof(btns_p2[0]);
                } else {
                    b = btns_p3;
                    nb = sizeof(btns_p3) / sizeof(btns_p3[0]);
                }
                for (int i = 0; i < nb; i++) {
