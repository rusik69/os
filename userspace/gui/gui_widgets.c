...
static void fb_draw(gui_widget_t *w) {
    ...
    /* Draw background */
    gui_rect_t rect = {fb->widget.rect.x, fb->widget.rect.y, fb->widget.rect.w, fb->widget.rect.h};
    ...
    /* Draw title (path) */
    gui_window_draw_text(NULL, rect.x + 4, rect.y + 4, fb->current_path, GUI_TEXT_FG, GUI_WHITE);
...
static void tb_draw(gui_widget_t *w) {
    ...
    /* Draw background */
    gui_rect_t rect = {tb->widget.rect.x, tb->widget.rect.y, tb->widget.rect.w, tb->widget.rect.h};
    for (int32_t y = rect.y; y < (int32_t)(rect.y + rect.h); y++) {
        for (int32_t x = rect.x; x < (int32_t)(rect.x + rect.w); x++) {
